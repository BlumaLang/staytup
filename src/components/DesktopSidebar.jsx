...
import { Compass, Search, Library, Users, User, Settings, Music2 } from 'lucide-react';
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-white' : 'text-[#8E8E93] group-hover:text-white'}`} />
        ...
        {/* Divider */}
        <div className="!my-3 border-t border-[#1C1C1E]" />

        {/* Friends */}
        <button
          onClick={onOpenFriends}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-[#8E8E93] hover:text-white hover:bg-white/5 transition-all group"
        >
          <Users className="w-5 h-5 flex-shrink-0 text-[#8E8E93] group-hover:text-white" />
          <span>Friends</span>
        </button>
      </nav>
